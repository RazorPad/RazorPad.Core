...
namespace RazorPad.ViewModels
{
	public static class StandardDotNetReferencesLocator
	{
		...
		private static void DiscoverKnownLocations()
		{
			...
			//foreach (var l in KnownRegistryLocations)
			//{
			//	d.AddRange(GetAssemblyFolders(l));
			//}

			_discoveredLocations = KnownRegistryLocations.SelectMany(GetAssemblyFolders)
														 .Union(KnownFolderLocations.Where(Directory.Exists));
		}
		...
		private static IEnumerable<string> GetAssemblyFolders(string knownLocation)
		{
			...
			if (key != null)
			{
				locations.Add(key.GetValue("").ToString());
